...
public class XMLParser {
    ...
    /**
     * Parses the XML file that was modified and populates the JIRAIssue data
     * object.
     * This JIRAIssue is then used to create or update the an issue on the JIRA
     * instance
    ...
    public void parseXML(JIRAIssue issue, Path filePath, String fileName, JiraController JIRAController) {
        try {
            ...
            // Searches for issue JIRA key by Contact ID of issue; returns null if it
            // doesn't exist
    ...
    public static void processDataNode(Node node, Document doc, JIRAIssue issue) throws IOException {
        switch (node.getNodeName()) {
            ...
            default:
                String value = node.getNodeName() == "AOS" | node.getNodeName() == "LOS"
                        ? Long.toString(dateToEpoch(node.getTextContent()))
                        : node.getTextContent();
